...
The project is built with [Bazel](https://bazel.build/) (Bzlmod). Use
[Bazelisk](https://github.com/bazelbuild/bazelisk) as the launcher; it reads the
pinned Bazel version from `.bazelversion`.

```sh
# Install the launcher (Go toolchain required); it lands in $GOPATH/bin.
go install github.com/bazelbuild/bazelisk@latest

bazelisk build //...        # build everything
bazelisk build //:taxinomia # build just the main binary
bazelisk run   //:taxinomia # build and run
bazelisk test  //...        # run all tests
```

After adding/removing `.go` or `.proto` files, regenerate the `BUILD.bazel`
files with Gazelle; after changing `go.mod`, sync the module deps:

```sh
bazelisk run //:gazelle     # regenerate BUILD files
bazelisk mod tidy           # sync go.mod deps into MODULE.bazel
```

Notes:
- **Version.** The display version is the commit count of the source
  revision (`r168`), shown in the UI's status bar and landing page, in the
  perf tab, and in the `X-Taxinomia-Version` response header. It is never
  edited by hand. A tracked pre-commit hook writes it into
  `core/buildinfo/version.go` at every commit, so it travels with the
  source to every importer — enable the hook once per clone:
  `git config core.hooksPath tools/hooks`. The Bazel build additionally
  stamps the binary from git (`.bazelrc` sets `--stamp` and a workspace
  status command under `tools/`), adding the short hash and a `+dirty`
  marker for builds from a modified tree. `go test ./core/buildinfo/`
  fails if a commit was made without the hook.
- Protobuf Go bindings (`*.pb.go`) are **generated by Bazel** via
  `proto_library` + `go_proto_library`, not checked in. `protoc` is downloaded
  prebuilt (via `toolchains_protoc`), so no local C/C++ toolchain is required.
  Because the bindings are not committed, plain `go build` / `go test` will not
  compile the protobuf-dependent packages — build through Bazel.
- **Windows:** `bazelisk run //:gazelle` needs a Unix shell. Set the `BAZEL_SH`
  environment variable to your `bash.exe` (e.g. Git for Windows:
  `C:\Program Files\Git\usr\bin\bash.exe`). Plain `build`/`test` do not need it.
