...
Notes:
- The binary is **stamped with its git revision** (`.bazelrc` sets `--stamp`
  and a workspace status command under `tools/`): the UI's status bar,
  landing page and the `X-Taxinomia-Version` response header show
  `r<commit count> · <short hash>`, `+dirty` when built from a modified
  tree, or `dev` for unstamped builds such as tests. Nothing to maintain by
  hand; every commit raises the number.
